...
@click.argument("path", type=click.Path(exists=False, writable=True, file_okay=False, dir_okay=True, resolve_path=True, path_type=Path))
...
@click.option("--dependencies", "-d", type=str, multiple=True, default=[], shell_complete=complete_available_migration_id)
def add_migration(path: Path, id: Optional[str], transactional: bool, idempotent: bool, dependencies: typing.List[str]):
    if path.exists():
        raise click.UsageError(f"Cannot create migration, because {path} already exists.")
    try:
        repository = Repository.from_closest_parent()
    except (FileNotFoundError, NotADirectoryError) as e:
        raise click.UsageError(str(e)) from e
...
def add_repository(path: Path, name: Optional[str]):
    if path.exists():
        raise click.UsageError(f"Cannot create repository, because {path} already exists.")
